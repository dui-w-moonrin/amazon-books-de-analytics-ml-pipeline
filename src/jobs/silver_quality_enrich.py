...
from src.transformers.config_driven_quality_enricher import (
    ConfigDrivenQualityEnricher,
)
...
class SilverQualityEnrichJob:
    ...
    def _resolve_output_path(self) -> Path:
        dataset_name = self._get_dataset_name()
        output_asset = self.config.get("output_asset", "silver_quality_enriched")
    ...
    def _create_spark_session(self) -> SparkSession:
        ...
        builder = (
            ...
            .appName(
                spark_config.get(
                    "app_name",
                    f"silver-quality-enrich-{dataset_name}",
                )
            )
            .config(
                "spark.driver.memory",
    ...
    def run(self) -> None:
        ...
        try:
            ...
            enricher = ConfigDrivenQualityEnricher(input_df, self.config)
            output_df = enricher.transform()
