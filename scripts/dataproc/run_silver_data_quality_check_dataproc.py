...
def main():
    ...
    from src.jobs.silver_data_quality_check import SilverDataQualityCheckJob

    patch_job_create_spark_session(
        SilverDataQualityCheckJob,
        "silver-dq-check",
    )
    ...
    job = SilverDataQualityCheckJob(project_root=project_root, config=config)
    job.run()
